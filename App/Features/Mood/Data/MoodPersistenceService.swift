...
final class MoodPersistenceService: ObservableObject {
    ...
    func saveMoodEntry(
        ...
        tags: Set<String>) async throws -> MoodEntry
    {
        let entry = MoodEntry(
            moodOption: moodOption,
            intensityLevel: intensityLevel,
    ...
    private func performBackgroundWrite(
        operation: @escaping @Sendable (ModelContext) throws -> Void) async throws
    {
        try await Task.detached(priority: .userInitiated) { @Sendable [modelContainer] in
    ...
    /// - Throws: Any errors from the operation
    private func performBackgroundFetch<T: Sendable>(
        operation: @escaping @Sendable (ModelContext) throws -> T) async throws -> T
    {
        try await Task.detached(priority: .userInitiated) { @Sendable [modelContainer] in
            let backgroundContext = ModelContext(modelContainer)
...
extension MoodPersistenceService {
    /// Backing container for SwiftUI previews that need a separate `ModelContext`.
    var modelContainerForPreview: ModelContainer { modelContainer }
